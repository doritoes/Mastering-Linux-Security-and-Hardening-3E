...
REM VBoxManage startvm %VM% --type gui
VBoxManage unattended install %VM% --iso=C:\LinuxSecBook\ISOs\AlmaLinux-9-latest-x86_64-dvd.iso --user=tux --full-user-name=doritoes --password="hardLINUXp@$$" --no-install-additions --hostname=ubuntu.mylinux.lab --country=US --language=en-US --locale=en_US --time-zone=ET --start-vm=gui
